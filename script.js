  ...
  try {
    const response = await fetch(webhookUrl, {
      ...
      headers: {
        ...
        // เรายังไม่ใส่ Authorization ตรงนี้นะครับ เพื่อดูว่า Make.com เรียกร้องหาอะไร
      },
      body: JSON.stringify({
    ...
    // --- ส่วนที่เพิ่มเข้ามาใหม่: การดักจับและแสดงรายละเอียด Error เชิงลึก ---
    if (!response.ok) {
      // อ่านข้อความ Error ที่ Make.com ส่งกลับมา
      const errorBody = await response.text(); 
      
      if (response.status === 401) {
         console.error('🚨 [Error 401]: Make.com ปฏิเสธการเข้าถึง');
      }
      
      // โยน Error ไปให้ catch ทำงาน พร้อมแนบสาเหตุที่แท้จริง
      throw new Error(`HTTP Status: ${response.status} | รายละเอียด: ${errorBody}`);
    }
    // --------------------------------------------------------
    ...
    // แสดง Error ที่ได้มาแบบละเอียดใน Console
    console.error('❌ สาเหตุที่พัง:', error);
    alert('เกิดข้อผิดพลาดในการเชื่อมต่อกับ AI กรุณาลองใหม่อีกครั้ง (ดูสาเหตุใน Console)');
